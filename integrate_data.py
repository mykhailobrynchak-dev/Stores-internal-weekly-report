...
for r in ops_overview:
    ...
    overview_cp_weekly.append({
        ...
        "ops_gmv_eur": r.get("ops_gmv_eur"),
        "commission_eur": r.get("commission_eur"),
        "cp_l1_eur": r.get("cp_l1_eur"),
        "cp_l2_eur": r.get("cp_l2_eur"),
        "courier_cost_eur": r.get("courier_cost_eur"),
...
for r in ops_overview:
    ...
    overview_ops_weekly.append({
        ...
        "availability_ent_pct": r.get("availability_ent_pct"),
        "availability_smb_pct": r.get("availability_smb_pct"),
        "availability_mm_pct": r.get("availability_mm_pct"),
...
for period in sorted(q_ops_groups.keys()):
    ...
    overview_cp_quarterly.append({
        ...
        "ops_gmv_eur": sum((r.get("ops_gmv_eur") or 0) for r in rows),
        "commission_eur": sum((r.get("commission_eur") or 0) for r in rows),
        "cp_l1_eur": sum((r.get("cp_l1_eur") or 0) for r in rows),
        "cp_l2_eur": sum((r.get("cp_l2_eur") or 0) for r in rows),
        "courier_cost_eur": sum((r.get("courier_cost_eur") or 0) for r in rows),
    ...
    overview_ops_quarterly.append({
        ...
        "availability_ent_pct": avg_vals(rows, "availability_ent_pct"),
        "availability_smb_pct": avg_vals(rows, "availability_smb_pct"),
        "availability_mm_pct": avg_vals(rows, "availability_mm_pct"),
...
for pname in partners_list:
    ...
    for qp in sorted(q_cp_groups_p.keys()):
        quarterly_cp.append({
            ...
            "ops_gmv_eur": sum((r.get("ops_gmv_eur") or 0) for r in q_cp_groups_p[qp]),
            "commission_eur": sum((r.get("commission_eur") or 0) for r in q_cp_groups_p[qp]),
            "cp_l1_eur": sum((r.get("cp_l1_eur") or 0) for r in q_cp_groups_p[qp]),
            "cp_l2_eur": sum((r.get("cp_l2_eur") or 0) for r in q_cp_groups_p[qp]),
            "courier_cost_eur": sum((r.get("courier_cost_eur") or 0) for r in q_cp_groups_p[qp]),
...
# ======== EMPLOYEE GROUPS ========
EMPLOYEE_GROUPS = {
    "Mykhailo": ["LOKO", "KOPIYKA", "HOP HEY", "BEER MARKET", "CAFE RYNOK", "TAISTRA", "BEERLAND K", "WINETIME", "BRSM", "SPAR", "OKKO MARKET", "ROST"],
    "Viktor": ["VARUS", "RUKAVYCHKA"],
    "Khrystyna": ["TOCHKA", "LEPRUKON", "MAXBEER", "SPRAGA", "DIMPYVA", "ALTBIER", "FLOWER SHOP", "NO TABOO", "RODYNNA KOVBASKA", "VAPERY | VAPE SHOP", "VAPORS", "PIVASOV", "MEALTIME", "CHILL TIME", "MILLION FLOWERS", "ANRI-PHARM"],
    "NEW w/o AM": ["AUCHAN", "ATB", "FLOWERS UA", "THRASH", "E-ZOO", "MASTER ZOO", "BYLE TA SYKHE", "FORA", "ANC", "BLYZENKO", "LIKI 24", "REMESLO BREWERY", "PYVNA BORODA", "EKO MARKET"],
}
