...
- Commission % / € and CM L1: `fact_provider_weekly`. Current WTD uses the week starting 10 August (partial week through 13 Aug at snapshot time). Prior week uses the full week starting 3 August (3–9 Aug). Commission € is the GMV-weighted commission amount; Commission % is Commission of GMV. CM L1 is `total_contribution_profit_eur` (same as CP Margin in the weekly report), shown in € and as a % of `total_gmv_before_discounts_eur`.
- Monetary values: EUR.
